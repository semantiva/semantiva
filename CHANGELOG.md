...
- Finalised pipeline and persona documentation around YAML-as-SSOT vs
  Python-as-R&D, introducing `pipelines_yaml` / `pipelines_python`, updating
...
### Summary
Semantiva v0.5.0 is the first public-ready release focused on stability, observability and deterministic pipeline identity. This release introduces stable tracing (SER v1), a run-space / parametric sweep system, a hardened inspection/identity model, and the `semantiva` CLI. 

**Important**: For first-time users we recommend starting from **semantiva v0.5.0**. Earlier versions (v0.4.0 and below) are discouraged for new projects.

### Highlights
- **Stable Tracing (SER v1):** normalized, compact trace records with lifecycle events and packaged JSON schemas for reliable tooling and integration.
- **Run-Space & Parametric Sweeps:** first-class run-space lifecycle, deterministic `derive.parameter_sweep` preprocessors, and support for product/zip sweep modes and tradeoffs.
- **Deterministic Identity & Inspection:** deterministic pipeline/config semantic IDs (`plsemid-*` and `plcid-*`) and a hardened, sanitized inspection payload (extended CLI view available).
- **CLI & Runtime polish:** `semantiva` console entry point, `run` and `inspect` subcommands, extended inspection and improved preflight checks.
- **Docs & Tests:** new trace/run-space documentation, example pipelines, and expanded unit/integration coverage.

### Other
- Various internal reorganizations and cleanups to prepare a stable, user-focused public API and developer experience.
