...
class ImageDataTypeDummySink(ImageDataTypeSink):
    ...
    This class accepts `ImageDataType` objects without performing any processing or saving operations.

    Use this class for testing or as a placeholder in pipelines where no action
    is required on the input data.
    """

    def _send_data(self, data: ImageDataType):
        """
        Accepts and validates input data as an `ImageDataType`.

        This method performs no processing or saving; it simply validates that the input data
        is of the correct type.

        Parameters:
            data (ImageDataType): The input data to the sink.

        Raises:
            ValueError: If the input data is not an `ImageDataType`.
        """
        if not isinstance(data, self.input_data_type()):
            raise ValueError(
                f"ImageDataTypeSink accepts only ImageDataType, but received {type(data).__name__}."
            )
...
class ImageStackDataTypeDummySink(ImageStackDataTypeSink):
    ...
    This class accepts `ImageStackDataType` objects without performing any processing or saving operations.

    Use this class for testing or as a placeholder in pipelines where no action
    is required on the input data.
    """

    def _send_data(self, data: ImageStackDataType):
        """
        Accepts and validates input data as an `ImageStackDataType`.

        This method performs no processing or saving; it simply validates that the input data
        is of the correct type.

        Parameters:
            data (ImageStackDataType): The input data to the sink.

        Raises:
            ValueError: If the input data is not an `ImageStackDataType`.
        """
        if not isinstance(data, self.input_data_type()):
            raise ValueError(
                f"ImageStackDataTypeSink accepts only ImageStackDataType, but received {type(data).__name__}."
            )
        # No action is performed on the data
        pass
...
class ImageDataTypePayloadDummySink(ImageDataTypePayloadSink):
    ...
    This class receives payloads without performing any processing or saving operations.
    It is designed for testing or as a placeholder in pipelines where no action
    is required on the input data.

    Methods:
        send_payload(payload: tuple[ImageDataType, ContextType]):
            Accepts the payload without performing any operation.
    """

    def _send_payload(self, data: ImageDataType, context: ContextType):
        """
        Accepts a payload containing ImageDataType and ContextType without processing it.

        Parameters:
            payload (tuple[ImageDataType, ContextType]):
                A tuple containing the ImageDataType object and its associated ContextType.

        """
        # No processing or saving operation is performed
        if not isinstance(data, self.input_data_type()):
            raise ValueError(
                f"ImageDataTypePayloadDummySink accepts only ImageDataType, but received {type(data).__name__}."
            )
        if not isinstance(context, ContextType):
            raise ValueError(
                f"ImageDataTypePayloadDummySink accepts only ContextType, but received {type(context).__name__}."
            )
        pass
