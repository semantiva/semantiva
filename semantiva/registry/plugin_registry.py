...
def load_specializations(specs_to_load: str | List[str]):
    ...
    for package in installed_packages:
        if package.name in specs_to_load:
            ...
            logger.debug("Subscribing %s to ClassRegistry paths", package.name)
            semantiva_spec().register()
            found_specs.add(package.name)
            logger.debug("Subscribed %s to ClassRegistry paths", package.name)
...
class SemantivaSpecialization(ABC):
    ...
    def register(self) -> None:
        """
        Register all modules, paths, etc in ClassRegistry.
        """
        pass
