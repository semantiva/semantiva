...
5. A string builder context factory creates a filename using the run-space
...
- The example uses a single ``zip`` block that pairs ``value``, ``addend`` and
  ``factor`` lock-step. Additional blocks could add seeds, augmentations, or any
...
- ``zip`` block: the lists under ``context`` must be the same length; each index
  becomes one run. ``product`` blocks compute full Cartesian products.
...
- SER provenance: each run records ``run_space.index`` and ``run_space.context``
  inside ``why_ok.args`` along with full block metadata for auditability.

Next steps
----------

- To experiment with combinatorial sweeps, add a second block with
  ``mode: product`` (for example seeds or augmentations).
- To plan without executing, set ``run_space.plan_only: true`` or pass
  ``--run-space-plan-only`` to :command:`semantiva run`.
